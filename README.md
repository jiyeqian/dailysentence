...
和乐词 App 的「个人信息卡片」模板合成为一张 **1080 × 1920** 的手机海报。
...
- 中英文句子、关键词、音标、词性释义、例句、常见用法、出处
- 网页「每日一句」卡片左上角的配图（`static.esdict.cn/MediaPool/daymonthimg/…`，每天轮换）
- 真人发音地址（正常 / 慢速，可试听）

**合成**

- 画布固定 `1080 × 1920`，浏览器 Canvas 完成，**模板图不上传**
- 个人信息卡片位置**自动识别**：近纯白二值化 + 连通域，取不接触画布边缘的最大白块，
  按圆角矩形抠出后重新贴上并重绘投影
- 三种背景模式：`铺满` / `模糊底 + 完整图` / `模糊底 + 图片卡片`
- 顶部文字块会**自动缩字号**（最低 70%）直到不压到卡片；底部面板高度按内容计算、贴底对齐，
  冲突时卡片自动上移避让
- 中文逐字断行、英文按词断行，并避免行首出现 `，。、；：？！）】》」』` 等标点
- 关键词用 Playfair Display 衬线大标题，正文 Inter + 苹方混排，底部毛玻璃解析面板
  （左侧渐变竖条、等宽音标、词性彩色胶囊 n./v./adj.）

**交互**

- 全部文案可手动改；可导入自己的模板图并「重新识别」
- 4 个滑块微调卡片位置（上边距 / 高度 / 左右边距）
- PWA，可添加到主屏幕；保存到相册
