...
`bg / badge-date / en / cn / source / card`。顶部图片默认**铺满裁切**；`en / cn / source`
作为一个整体在中部区域里**垂直居中、并自动放大到几乎占满**：
...
| id | 中文名 | 版本 | 备注 |
| --- | --- | --- | --- |
| `bg` | 背景图区 | 都有 | 标准版 = 顶部固定 648px 那块（默认铺满裁切）；长版 = 整张画布或图片自然高度 |
| `badge-date` | 日期胶囊 | 都有 | 中部区域顶部靠右（右线对齐 48）；双击可把它从海报上删掉 |
| `en` / `cn` | 英文句 / 中文句 | 都有 | 左线 48；双击即删；**在这两个或 `source` 上上下拖动 = 缩放字号** |
| `source` | 出处 | 都有 | `—— ` 前缀一起算在框里 |
...
| 想要的效果 | 你自己怎么操作 |
| --- | --- |
| 删掉英文 / 中文 / 出处里某一块 | **双击它**；剩下的内容会自动放大填补（`text.autoScale` 会变大） |
| 中部字号整体大一点 / 小一点 | 在英文、中文或出处上**上下滑动**（上滑放大、下滑缩小，改的是 `text.zoom`）；刷新或点顶栏「重新获取」回到自动值 |
| 换顶部图片 / 信息卡 | **单击**那一块，从相册选图 |
| 回到最初始的样子 | 点顶栏「重新获取」（等同重启 App） |
