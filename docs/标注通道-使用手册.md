...
`unclipped` = 预览按「不裁切」画，即窗口外能看到这张图的其余部分；`shiftY` = 为了让该区居中，
海报当前的竖直位移，退出后归 0），`fits` 是两块图各自的缩放与位移
（`scale = 1, ox = oy = 0` = 刚换上的默认样子）。

> **调整模式长什么样**：正在调的那块被平移到**屏幕中央**（尺寸与成品一致、不放大），
> 窗口外一层约 62% 的半透明遮罩，透过它能看到这张图在窗口外的其余部分（拖动 / 缩放的引导）；
> 海报的句子与信息卡仍在遮罩下隐约可见。**没有任何描边 / 虚线** —— 亮暗交界就是区域边界
> （历史上的青色虚线框已删除）。
...
`{ active: true, target: "en", style: "bars", since }` —— 此刻**只有那块波形层可点**，
它是播放期唯一的停止按钮（点它就停、波形消失、其余区域立刻恢复），其它区域一律不可操作
（不保存、不更新、不缩放、不删除、不弹相册）。它盖在 3 区（英文句）上，半透明，
所以句子仍读得出来；面板是 DOM 浮层，**不进 canvas**，也不进 `?raw=1` 的导出图。
...
| 调整刚换上的图 | 换完图后会立刻进入调整模式：该区会**移到屏幕中央、周围压暗**（压暗处透出这张图的其余部分当引导），**单指拖动**挪图、**双指捏合**缩放、**单击图片 / 卡片**换一张，**点被调区域以外**即完成（保存 / 更新 / 朗读 / 删除让路） |
