...
```json
{
  ...
  "items": [
    { "i": 4, "id": "cn", "label": "中文句",
      "box": [48, 1044, 935, 161.2], "font": 55.2,
...
| 字段 | 含义 |
| --- | --- |
| `text.fx` | **2/3/4/5 各区自己的交互倍率**（1 = 设计基准字号；`badge-date` / `en` / `cn` / `source`）。上滑放大、下滑缩小，范围 0.6–1.6；3/4/5 联动、2 区独立 |
...
> 3/4 区 55.2、5 区 38.1、2 区（日期胶囊）27.5 —— 这是「进入应用即显示」的固定值，
> 不再随句子长短自动放大或缩小。
...
> **中部文字的字号是固定设计基准**（2026-09-22 起）：3/4 区 55.2、5 区 38.1、2 区日期 27.5 ——
> 进入就按这个显示，**不再自动放大铺满**，所以「太空 / 太挤」在这块是正常且每天的观感一致；
> 短句不会被撑大，长句也只在整块装不下时才整块等比缩小（`text.base < 1`）。
> 想调就自己上下滑动（见 `text.fx`），或者直接报「3 区字号 55.2 → 50」这类具体数字。
...
- 「中部整体再大一点」→ 我调默认的设计基准字号 `SZ_EN` / `SZ_CN` / `SZ_SOURCE`
...
| 某个文字区大一点 / 小一点 | 在那一区上**上下拖动**（上滑放大、下滑缩小，改的是该区的 `text.fx`）；**3/4/5 联动、2 区独立**；刷新或下拉更新回到设计基准 |
...
注意 `字号` 那一列：标准版文字区用的是**新的设计基准**（3/4 区 55.2、5 区 38.1、2 区日期 27.5），
