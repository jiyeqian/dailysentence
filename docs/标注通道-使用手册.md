...
> **跨设备不用换算**：手机上实际出图是设备分辨率（如 1180×1978），但清单里的坐标**永远以 1080 为基准**
> —— `canvas.u` 是位图 ÷ 设计宽 的倍数，`canvas.physW/physH` 是设备位图尺寸、
> `canvas.bitmapW/bitmapH` 是画布缓冲实际尺寸（长版按内容长高，会比 physH 更高），
> `canvas.adaptive` 表示是否走了手机自适应。报编号 / 报坐标时不用关心设备。
