...
编号是双方共用的坐标。你看到的是「4 号」，我读到的是 `cn` 的 `box [84, 943, 882, 123]`、字号 `42`。
...
`bg / badge-date / en / cn / source / card`：
...
```json
{
  ...
  "items": [
    { "i": 4, "id": "cn", "label": "中文句",
      "box": [84, 943, 882, 123], "font": 42,
      "color": "rgba(255,255,255,0.88)", "text": "伟大的事不是凭一时冲动完成的…", "kind": "item" }
  ],
  "gaps": [
    { "id": "img-h", "label": "顶部图片区高度", "px": 648 },
    { "id": "gap-img-text", "label": "图片区 → 文字块", "px": 64 }
  ],
  "opts": { "bgStyle": "natural", "longPoster": false },
...
| id | 中文名 | 版本 | 备注 |
| --- | --- | --- | --- |
| `bg` | 背景图区 | 都有 | 标准版 = 顶部固定 648px 那块；长版 = 整张画布或图片自然高度 |
| `badge-date` | 日期胶囊 | 都有 | 右上角；双击可把它从海报上删掉 |
| `en` / `cn` | 英文句 / 中文句 | 都有 | 双击即删 |
| `source` | 出处 | 都有 | `—— ` 前缀一起算在框里 |
| `card` | 个人信息卡 | 都有 | 标准版固定在 x=48、984×496 |
...
| `gap-img-text` | 图片区 → 文字块 | 标准版（64） |
| `gap-text-card` | 文字块 → 信息卡（自动撑开的余量） | 标准版 |
| `margin-x` | 左右边距：标准版 = 信息卡距左右（48）；长版 = 文字安全边距（84） | 都有 |
...
你：1 号矮一点；3、4 之间松一点；6 号底边别那么紧
我：（改代码）→ 跑 --tag v2 → 跑 --diff latest v2

      ~ bg    背景图区（原比例）  Δbox=[0, 0, 0, -88]
      ~ cn    中文句           Δbox=[0, +16, 0, 0]
...
~ badge-date  日期胶囊  Δbox=[0, +4, 0, 0]
~ en  英文句  Δbox=[0, +112.2, 0, 0]
~ cn  中文句  Δbox=[0, +112.1, 0, 0]
~ source  出处  Δbox=[0, +112.2, 0, 0]
