...
- **功能全部由海报上的手势承载**（唯一提示是首次进入 3 秒后自动消失的一行 `#hint`）：
  ...
  | 在文字区（2 日期 / 3 英文 / 4 中文 / 5 出处）上上下拖动 | 实时缩放该区字号（0.6–1.6 倍；**3/4/5 联动、2 区独立**） |
...
- **字号是固定设计基准，不再自适应放大**（2026-09-22 用户重定）：标准版 3/4 区 55.2、
  5 区 38.1、2 区（日期胶囊）27.5（推导见 `SZ_*` 常量注释）—— 进入应用就是这个字号，
...
- 日期胶囊高度恒为 `DATE_H`，**不随 2 区的缩放长高** —— 它一变高就会改活动区高度、
  让句子被保底缩小，那就破坏了「2 区独立」。
