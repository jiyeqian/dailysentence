...
- 交互只有三种手势：
  ...
  - **长按海报 = 保存**
- **标准版画布恒 1080×1920**，句子装不下只自动缩文字块字号，绝不长高；
  信息卡距左 / 右 / 底三边等距 48px。长版（`?long=1`）保持旧版面与长高逻辑。
- `state.regions` 的 id、`window.__ds.inspect()` 的字段形状，是 `inspect.js` 与
  `ui-check.js` 的契约，改交互时别改这两处的对外形状。
