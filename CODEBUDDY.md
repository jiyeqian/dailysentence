...
- **版面一律用「设计坐标」写：宽恒 1080，高 `CH`**。实际位图 = 设计坐标 × `U`，
  由 `render()` 里一次 `ctx.setTransform(U,0,0,U,0,0)` 完成缩放 ——
  **所以尺寸 / 字号 / 间距都不用乘比例，改版式按 1080 基准写就行**。
  - 桌面（`pointer: coarse and hover: none` 不成立，或 1x 屏幕）：`U = 1`，位图就是 1080×1920
  - 手机 / PWA：位图 = 屏幕比例、宽度不低于 1080（`max(1080, 短边×dpr)`，取偶数），
    `U = 位图宽 / 1080`，`CH_MIN = 位图高 / U` → 部件等比放大、更高的屏幕把多出来的高度
    全给句子活动区。保存出来的就是设备原生分辨率的竖版海报
  - 画布取「短边 × 长边」而不是当前视口宽高 → **旋转屏幕时画布尺寸不变、版式不跟着跳**
    （横屏等比缩小居中；横屏版面未来单独设计）
  - 设备安全区折算成设计坐标后画进画布：`SAFE.bottom` 加在信息卡底边距上，
    顶部图片顶到屏幕顶（手机端 `#stage` 内边距归零），这样才是真全屏
  - 极端比例（平板 3:4、折叠屏）有压缩优先级：图片区 ≤ 画布 45%、卡片 ≤ 28%、
    句子区至少 28%，不够就继续压图片区
