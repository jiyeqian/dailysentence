...
- **换图后自动进入「手动调整模式」**（`state.edit = { target: 'img'|'card' }`）：
  ...
  **点被调区域以外的地方 = 完成**。别给它加按钮，也别改这套语义。
  - **界面是「裁剪编辑器」式的**（2026-09-22 重做）：正在调的那块被**平移到屏幕中央**
    （`#poster` 加 CSS `translateY`，尺寸不变、所见即所得），窗口**外**一层
    `rgba(6,10,18,0.62)` 的半透明遮罩，透过它能看见**这张图在窗口外的其余部分**
    （`drawBackground` / `drawProfileCard` 在调整时跳过 clip）—— 那是拖动 / 缩放的引导。
  - **不许有任何描边 / 虚线**：亮暗交界就是这块区域的边界（用户明确要求）。历史上的
    `drawEditFrame()`（青色虚线框）已删除，回归里有一条**源码级断言**盯着它不许回来。
  - 平移必须走 **CSS transform**，不要在 canvas 里 `ctx.translate`：纯平移不改变画布 rect 的
    宽，且 `getBoundingClientRect()` 会带上位移，所以 `toClient` / `hitTestAt` / `pxToCanvas`
    全部自动正确、「点区域外完成」不会错位；在 canvas 里平移会让整张命中表跟着错位。
    画布位图也仍是干净成品，长按另存与 `?raw=1` 导出都不受影响。
  - 位置（位移量、遮罩洞）只在 **`render()` 末尾的 `layoutEditView()`** 里算一处，
    别在 `startEdit` 里另算一份；视口变化时靠 `watchViewport()` 的防抖重排（画布尺寸不变也要重排）。
  - 该区居中后，海报下半部分（含信息卡）会移出屏幕 —— 这是居中换来的代价，正常现象；
    「点另一块换图」要点它露在屏幕里的那一段。
