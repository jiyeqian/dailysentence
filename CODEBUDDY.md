...
- **长版与标准版交互同权（2026-09-24 迁移）**：`?long=1` 不再是「纯展示」—— 朗读 /
  日期切换 / 双击删除 / 上下拖缩字号 / 信息卡换图 / 下拉更新 / 保存全部可用；
  字号基准沿用长版自己的 42/29/25，**没有「保底缩小」**（靠画布长高装下，`textFit` 只属
...
- **版式（standard/long）双指扩/捏切换**：扩 ≥1.6 倍 标准→长版，捏 ≤0.62 倍 长版→标准；
  ...
  toast、写 `localStorage['ds:mode']`。启动优先级 **URL `?long=1` 强制 > 记忆 > 标准版**
  （`initMode()`）；下拉更新只复位内容**不换版式**。切版后画布高度变化走 render 现成的
  resize 路径（长版内容矮于 1920 时 CH = 1920，与标准版同高是正常的）。
